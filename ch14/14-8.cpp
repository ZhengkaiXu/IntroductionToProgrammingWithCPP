...
#include <sstream>
#include <cstdlib> // For the bas function
Rational::Rational() {
    numerator = 0;
    denominator = 1;
}
...
ostream& operator<<(ostream& out, const Rational& raitonal) {
    if (raitonal.getDenominator() == 1) {
        out << raitonal.numerator;
    } else {
        out << raitonal.numerator << "/" << raitonal.denominator;
    }
    return out;
}
